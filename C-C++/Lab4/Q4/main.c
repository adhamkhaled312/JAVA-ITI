...
int main() {
    ...
    gotoxyCenter(30, 0);
    scanf_s("%d", &employeeSize);
    employeeData = (Employee*)malloc(employeeSize * sizeof(Employee));
    while (!terminated) {
        system("cls");
        for (int i = 0; i < n; i++) {
            gotoxyCenter(0, i);
        ...
        case KEY_ENTER:
            ...
            case NEW_OPTION:
                if (employeeCount == employeeSize) printf("Maximum number of employees reached.");
                else readEmployee();
                gotoxyCenter(0, 1);
